...
function discover(){
 ...
 # curl -s "https://api.certspotter.com/v1/issuances?domain=$domain&include_subdomains=true&expand=dns_names" | jq .[].dns_names | tr -d '[]"\n ' | tr ',' '\n' | grep "$domain" | grep -v "*" | sed -e 's/^[[:punct:]]//g' | sed -r '/^\s*$/d' | anew $subdomains
  ...
  echo "${green}[+] Subfinder ...${reset}"
  subfinder -d $domain -silent | anew $subdomains
  
  echo "${green}[+] Assetfinder ...${reset}"
  assetfinder --subs-only $domain | anew $subdomains
  
  echo "${green}[+] Findomain ...${reset}"
  findomain -t $domain -q | anew $subdomains
  
}

main(){
  if [ -f "./getall-$domain.txt" ]; then
    echo -e "${red}[!] Apparently you've already searched subdomains on this target.${reset}"
    echo -e "${red}[!] File '$(pwd)/getall-$domain.txt' already exists.\n"
    tput cnorm; exit 1
  else
    touch ./getall-$domain.txt
  fi
  subdomains="./getall-$domain.txt"
...
if [ $# -lt 1 ]; then
  ...
  echo "Usage: ./getallsubdomains.sh <domain>"
  tput cnorm; exit 1
else
  if [ $# -ge 2 ]; then
